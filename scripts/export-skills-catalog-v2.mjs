...
function buildLegacyTags(skillName, summary, frontmatter, mapping) {
  ...
  if (isPlatformNative) {
    metaTags.push('Source: MedAutoData');
  }
...
function inferSource(skill) {
  ...
  if (explicitSource === 'vibelab' || explicitSource === 'medautodata' || explicitSource === 'medautodata') {
    return 'medautodata';
  }

  const normalizedSource = normalizeKey(skill.source);
  return normalizedSource === 'vibelab' || normalizedSource === 'medautodata' || normalizedSource === 'medautodata'
    ? 'medautodata'
    : 'imported';
}

function inferStatus(skill, source) {
  if (source === 'vibelab' || source === 'medautodata') {
    return 'verified';
  }
...
async function main() {
  ...
  for (const skillFile of skillFiles) {
    ...
    rawSkills.push({
      ...
      source: isPlatformNative ? 'medautodata' : 'imported',
      summary,
      fullDescription,
      tags: {
        stage: stageTags,
