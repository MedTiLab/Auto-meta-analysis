...
const TaskList = ({ 
  ...
  const handleInitializeTaskMaster = async () => {
    if (!currentProject || isInitializingTaskMaster) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.message || payload?.error || 'Failed to initialize TaskMaster');
      }
      ...
      window.alert(error instanceof Error ? error.message : 'Error initializing TaskMaster. Please try again.');
    } finally {
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'title':
          ...
          break;
        case 'status':
          // Custom status ordering: pending, in-progress, review, done, blocked, deferred, cancelled
          const statusOrder = { pending: 1, 'in-progress': 2, review: 3, done: 4, blocked: 5, deferred: 6, cancelled: 7 };
  ...
  const kanbanColumns = useMemo(() => {
    ...
    const mainWorkflowStatuses = ['pending', 'in-progress', 'review', 'done'];
        ...
        {!hasTaskMasterDirectory ? (
          ...
          <div className="max-w-md mx-auto">
            ...
            <button
              ...
              className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors flex items-center gap-2 mx-auto"
            >
              <Settings className={`w-4 h-4 ${isInitializingTaskMaster ? 'animate-spin' : ''}`} />
