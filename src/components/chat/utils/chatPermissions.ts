...
export function grantToolPermission(entry: string | null, provider?: string): PermissionGrantResult {
  ...
  authenticatedFetch(`/api/settings/agent-permissions/${encodeURIComponent(provider || 'claude')}/allowed-tools`, {
    method: 'POST',
    body: JSON.stringify({ entry }),
  }).catch((error) => {
    console.error('Failed to persist agent permission:', error);
  });
