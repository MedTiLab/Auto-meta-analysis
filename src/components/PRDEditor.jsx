...
import { buildDefaultPrdFileName, getInitialPrdFileName, normalizePrdFileName } from '../utils/prdFileName';
...
- Keep planning and execution centered on the Claude workspace.
...
- Claude workspace integration for in-editor task management
...
- Claude workspace integration with AI task suggestions
...
- Webhook system for project notifications
...
- Approved By: Product Owner, Technical Lead`;
  ...
  useEffect(() => {
    const initializeEditor = async () => {
      setFileName(getInitialPrdFileName(file?.name, isNewFile));
  ...
  useEffect(() => {
    const fetchExistingPRDs = async () => {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          const prdFiles = data.prdFiles || data.prds || [];
          console.log('Fetched existing PRDs:', prdFiles);
          setExistingPRDs(prdFiles);
        } else {
  ...
  const handleSave = async () => {
    ...
    const fullFileName = normalizePrdFileName(fileName);
    ...
    console.log('Save check:', {
      ...
      fileObject: file,
      shouldShowModal: existingFile && !isExistingFile
    });
    
    if (existingFile && !isExistingFile) {
  ...
  const performSave = async () => {
    ...
    try {
      ...
      if (response2.ok) {
        ...
        setExistingPRDs(data.prdFiles || data.prds || []);
      }
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    const downloadFileName = normalizePrdFileName(fileName) || buildDefaultPrdFileName();
                ...
                <div className="flex items-center gap-1 min-w-0 flex-1">
                  <div className="flex items-center min-w-0 flex-1 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-md px-3 py-2 focus-within:ring-2 focus-within:ring-purple-500 focus-within:border-purple-500 dark:focus-within:ring-purple-400 dark:focus-within:border-purple-400">
                    <input
                      ...
                      placeholder="Enter filename (.txt, .md, .json)"
                      maxLength={100}
                    />
                  </div>
                  <button
                    onClick={() => document.querySelector('input[placeholder="Enter filename (.txt, .md, .json)"]')?.focus()}
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full border border-gray-200 dark:border-gray-700">
            <div className="p-6">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
                A PRD file named "{normalizePrdFileName(fileName)}" already exists. 
