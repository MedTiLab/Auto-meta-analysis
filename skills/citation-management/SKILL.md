...
## MedHelp Meta Smart-Screening Handoff

When this skill is used inside a numbered MedHelp Meta project, search/dedupe output must feed the AI screening chain explicitly:

- Source-specific skills write raw normalized records under `02_search_dedupe/search/imported_records/<source>.csv`.
- `citation-management` owns the final deduped AI input table: write exactly `02_search_dedupe/screening_input.csv` after duplicate resolution. CSV is preferred because agents can inspect and edit it reliably; `screening_input.json`/`.tsv` are compatibility alternatives. Do not include project names or timestamps in this final filename.
- Use CSV headers or a root JSON array / `{ "databaseName": "<source>", "records": [...] }`; include `title`, `doi`, `pmid` or `source_id`, `year`, `journal`, `abstract`, `authors`, and source metadata when available.
- These records are only the AI pre-screen input pool. They should be synced/linked to the project, but they should not be described as visible smart-screening results.
- Visible smart-screening results require exactly `03_title_abstract_screening/screening_decisions.csv` or `screening_decisions.json` with AI/Claude/user decisions. First-pass AI screening uses `reviewer: "ai_pre_screen"`; Claude review uses `reviewer: "claude"`.
- Do not overwrite `reviewer: "user"` decisions during dedupe or re-sync.
