...
| `instance.json` | `<project_path>/instance.json` | Paths are **absolute** when created by MedHelp (`Experiment.code_references`, `Ideation.references`); use as-is or resolve with `path.join(project_path, value)` if relative. Also `date_limit` from context. |
...
2. Write `Experiment/code_references/logs/repo_acquisition_agent.json`:
   ...
   {
     "context_variables": {
       "code_references_path": "<instance.Experiment.code_references if absolute (MedHelp), else path.join(project_path, ...)>",
       "references_path": "<instance.Ideation.references if absolute (MedHelp), else path.join(project_path, ...)>",
