...
- **Primary stack: Python** — use matplotlib, seaborn, plotly, and scikit-survival (or equivalents) as appropriate; follow **`scientific-visualization`** and related skills under `.agents/skills/` when they apply.
...
3. After collecting all information, use the `meta-pipeline-planner` skill (read `.agents/skills/meta-pipeline-planner/SKILL.md`) to generate the research brief and task pipeline.
...
Read `.agents/skills/meta-pipeline-planner/SKILL.md` and follow its procedure in any of these situations:
