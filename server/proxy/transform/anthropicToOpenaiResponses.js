 ...
 */
export function anthropicToOpenaiResponses(body, options = {}) {
    const input = [];
        ...
        if (block.type === 'text' || block.type === 'image') {
            ...
            const resultContent = typeof block.content === 'string'
                ...
                : Array.isArray(block.content)
                    ? block.content.filter((part) => (part.type === 'text' || part.type === 'image')).map(convertContentBlock)
                    : '';
 ...
 */
function isSelectableToolChoice(choice, tools) {
