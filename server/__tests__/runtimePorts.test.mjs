...
afterEach(async () => {
  ...
  delete process.env.MEDAUTODATA_RUNTIME_DIR;
  delete process.env.MEDAUTODATA_RUNTIME_FILE;
});
...
describe('runtimePorts', () => {
  ...
  it('reads runtime ports from an override directory and ignores stale process entries', async () => {
    const runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'medautodata-runtime-'));
  ...
  it('prefers medautodata runtime overrides over legacy dr-claw overrides', async () => {
    const legacyRuntimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'dr-claw-runtime-'));
    const currentRuntimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'medautodata-runtime-'));
    process.env.DR_CLAW_RUNTIME_DIR = legacyRuntimeDir;
    process.env.MEDAUTODATA_RUNTIME_DIR = currentRuntimeDir;
