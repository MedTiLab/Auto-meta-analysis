...
import { getConfiguredContextWindow } from '../shared/modelConstants.js';
...
function showStatus() {
    ...
    console.log(`       ${c.dim(process.env.MEDAUTODATA_DATA_DIR || resolveAppDataRoot())}`);
    ...
    console.log(`       CONTEXT_WINDOW: ${c.dim(process.env.CONTEXT_WINDOW || `${getConfiguredContextWindow()} (default)` )}`);
    ...
    console.log('\n' + c.dim('═'.repeat(60)));
    console.log(`\n${c.tip('[TIP]')} Hints:`);
    console.log(`      ${c.dim('>')} Use ${c.bright('medautodata --port 8080')} to run on a custom port`);
    console.log(`      ${c.dim('>')} Use ${c.bright('medautodata --database-path /path/to/db')} for custom database`);
    console.log(`      ${c.dim('>')} Run ${c.bright('medautodata help')} for all options`);
...
Examples:
  $ medautodata                        # Start with defaults
  $ medautodata --port 8080            # Start on port 8080
  $ medautodata -p 3000                # Short form for port
  $ medautodata start --port 4000      # Explicit start command
  $ medautodata status                 # Show configuration
...
Documentation:
  ${packageJson.homepage || 'https://github.com/YzGLab/medautodata'}

Report Issues:
  ${packageJson.bugs?.url || 'https://github.com/YzGLab/medautodata/issues'}
`);
}
...
async function checkForUpdates(silent = false) {
    try {
        ...
        const packageName = packageJson.name || 'medautodata';
...
async function updatePackage() {
    try {
        ...
        console.log(`${c.tip('[TIP]')} Try running manually: npm update -g ${packageJson.name || 'medautodata'}`);
    }
}
...
async function main() {
    ...
    switch (command) {
        ...
        default:
            ...
            console.log('   Run "medautodata help" for usage information.\n');
            process.exit(1);
    }
}
