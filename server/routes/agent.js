...
import { assertAccountUsageAvailable, USAGE_QUOTA_EXCEEDED_ERROR } from '../utils/accountUsage.js';
import { enforceClaudeModelForMembership } from '../../shared/modelConstants.js';
...
const validateExternalApiKey = (req, res, next) => {
  ...
  if (IS_PLATFORM) {
    try {
      ...
      assertAccountUsageAvailable(user);
      req.user = user;
      return next();
    } catch (error) {
      if (error?.message === USAGE_QUOTA_EXCEEDED_ERROR) {
        return res.status(403).json({ error: error.message, usage: error.usage });
      }
 ...
 * POST /api/agent
 *
 * Trigger Claude to work on a project.
 ...
 *
 * @param {string} model - (Optional) Claude model identifier.
 *
...
router.post('/', validateExternalApiKey, async (req, res) => {
  ...
  try {
    ...
    const membershipPlan = req.user?.membership_plan || 'free';
    const enforcedModel = enforceClaudeModelForMembership(model, membershipPlan);

    await queryClaudeSDK(message.trim(), {
      projectPath: finalProjectPath,
      cwd: finalProjectPath,
      sessionId: null,
      model: enforcedModel.model,
      membershipPlan,
      env: process.env,
      userId: req.user?.id,
      permissionMode: 'default',
      toolsSettings: {
        allowedTools: ['Read', 'Grep', 'Glob', 'Edit', 'MultiEdit', 'Write', 'TodoRead', 'TodoWrite'],
    ...
    if (createBranch || createPR) {
      try {
        ...
        if (createPR) {
          ...
          prBody += '\n\n---\n*This pull request was automatically created by MedAutoData Agent.*';
