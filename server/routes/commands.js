...
import {
  ...
  getClaudeModelOptionsForMembership,
  getConfiguredContextWindow,
} from '../../shared/modelConstants.js';
import { resolveProjectClaudeMemoryPath } from '../templates/index.js';
...
const builtInCommands = [
  ...
  {
    name: '/memory',
    description: 'Open the Claude project memory file for editing',
    namespace: 'builtin',
    metadata: { type: 'builtin' }
  },
  {
...
`;
  ...
  '/model': async (args, context) => {
    const membershipPlan = context?.membershipPlan || context?.membership_plan || 'free';
    const enforcedModel = enforceClaudeModelForMembership(context?.model || CLAUDE_MODELS.DEFAULT, membershipPlan);
    // Read available models from centralized constants
    const availableModels = {
      claude: getClaudeModelOptionsForMembership(membershipPlan).map(o => o.value),
    };

    const currentProvider = context?.provider || 'claude';
    const currentModel = enforcedModel.model;
    ...
    return {
      ...
      data: {
        ...
        message: args.length > 0
          ? `Your membership can only use model plan: ${currentModel}`
  ...
  '/cost': async (args, context) => {
    ...
    const model = enforceClaudeModelForMembership(
      context?.model || CLAUDE_MODELS.DEFAULT,
      context?.membershipPlan || context?.membership_plan || 'free',
    ).model;
    ...
    const total =
      Number(
        tokenUsage.total ??
          ...
          getConfiguredContextWindow(process.env.CONTEXT_WINDOW),
      ) || getConfiguredContextWindow(process.env.CONTEXT_WINDOW);
  ...
  '/status': async (args, context) => {
    ...
    return {
      ...
      data: {
        ...
        model: enforceClaudeModelForMembership(
          context?.model || CLAUDE_MODELS.DEFAULT,
          context?.membershipPlan || context?.membership_plan || 'free',
        ).model,
  ...
  '/memory': async (args, context) => {
    ...
    if (!projectPath) {
      return {
        ...
        data: {
          ...
          message: 'Please select a project to access its Claude project memory file'
        }
      };
    }

    const claudeMdPath = await resolveProjectClaudeMemoryPath(projectPath);

    // Check whether the preferred or legacy Claude project memory file exists.
    let exists = false;
    try {
    ...
    return {
      ...
      data: {
        ...
        exists,
        message: exists
          ? `Opening Claude project memory at ${claudeMdPath}`
          : `Claude project memory not found at ${claudeMdPath}. Create it to store project-specific instructions.`
      }
    };
  },
    ...
    if (handler) {
      try {
        const commandContext = {
          ...context,
          membershipPlan: req.user?.membership_plan || context?.membershipPlan || 'free',
        };
        const result = await handler(args, commandContext);
        return res.json({
          ...result,
          command: commandName
        });
