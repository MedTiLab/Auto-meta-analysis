 ...
 *
 * Current expected default root after the rename pass: ~/medautodata.
 *
...
test.describe('Workspace Path Customization', () => {
  ...
  test('API accepts workspace creation under ~/Documents (outside ~/medautodata)', async ({ request }) => {
    const token = await getAuthToken(request);
    const customPath = path.join(os.homedir(), 'Documents', `medautodata-test-${Date.now()}`);
  ...
  test('API accepts workspace creation under ~/medautodata (still works)', async ({ request }) => {
    const token = await getAuthToken(request);
    const validPath = path.join(os.homedir(), 'medautodata', `pw-test-${Date.now()}`);
  ...
  test('new project initializes Promotion directories and canonical instance.json paths', async ({ request }) => {
    const token = await getAuthToken(request);
    const projectPath = path.join(os.homedir(), 'medautodata', `pw-promotion-${Date.now()}`);
    ...
    try {
      ...
      expect(instance.Publication).toEqual({
        manuscript: path.join(projectPath, 'Publication', 'manuscript'),
        figures: path.join(projectPath, 'Publication', 'figures'),
        tables: path.join(projectPath, 'Publication', 'tables'),
        supplementary: path.join(projectPath, 'Publication', 'supplementary'),
      });
  ...
  test('API still rejects forbidden system paths', async ({ request }) => {
    ...
    const response = await request.post('/api/projects/create-workspace', {
      headers: { Authorization: `Bearer ${token}` },
      data: { workspaceType: 'new', path: '/etc/medautodata-test' },
    });
