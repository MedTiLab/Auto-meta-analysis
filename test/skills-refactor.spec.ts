 ...
 * - The remaining "Import Local Skills" button opens a browser upload modal
...
const REGISTRATION_AUTH_CODE = process.env.PLAYWRIGHT_REGISTRATION_AUTH_CODE || process.env.REGISTRATION_AUTH_CODE;
...
async function getAuthToken(request: APIRequestContext): Promise<string> {
  ...
  if (authStatus.needsSetup) {
    if (!REGISTRATION_AUTH_CODE) {
      throw new Error('Initial setup requires PLAYWRIGHT_REGISTRATION_AUTH_CODE or REGISTRATION_AUTH_CODE.');
    }
    ...
    const registerResponse = await request.post('/api/auth/register', {
      data: {
        ...
        notificationEmail: process.env.PLAYWRIGHT_SETUP_EMAIL || `${setupUsername}@example.com`,
        authorizationCode: REGISTRATION_AUTH_CODE,
      },
    });
...
test.describe('Skills Refactor — Single Import Button + No Project Skills', () => {
  ...
  test('4. Import modal opens as a browser upload dialog', async ({ page }) => {
    await goToSkillsDashboard(page);
    ...
    // Modal should appear
    const modalTitle = page.getByText(/Upload Skill|上传技能包/);
    await expect(modalTitle).toBeVisible({ timeout: 5000 });

    // Browser upload controls should be visible; server-local path scanning should be gone.
    await expect(page.getByText(/Drag and drop a \.zip file|拖拽 \.zip 文件/)).toBeVisible();
    await expect(page.locator('input[type="file"]')).toHaveCount(1);
    await expect(page.locator('input[placeholder="~/.claude/skills"]')).toHaveCount(0);
    ...
    console.log('PASS: Import modal opens with browser file upload controls');
  });

  test('5. Upload workflow exposes a zip file picker instead of scan controls', async ({ page }) => {
    await goToSkillsDashboard(page);

    // Open modal
    const importBtn = page.getByRole('button', { name: /Import Local Skills|导入本地技能/ }).first();
    await importBtn.click();
    await expect(page.getByText(/Upload Skill|上传技能包/)).toBeVisible({ timeout: 5000 });

    await page.screenshot({ path: 'test-results/skills-04-scan-result.png', fullPage: true });

    const fileInput = page.locator('input[type="file"]');
    await expect(fileInput).toHaveAttribute('accept', /zip/i);
    await expect(page.getByRole('button', { name: /Upload Skill|上传技能/ })).toBeDisabled();
    await expect(page.getByRole('button', { name: /^Scan$|^扫描$/ })).toHaveCount(0);

    console.log('PASS: Upload flow exposes a .zip file picker and no scan controls');
  });
